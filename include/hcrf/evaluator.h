...
public:
  ...
  virtual void computeStateLabels(DataSequence* X, Model* m, iVector*
                                  vecStateLabels, dMatrix * prob = NULL);
  virtual int computeSequenceLabel(DataSequence* X, Model* m, dVector* prob,
                                   dMatrix* hprob = NULL);
...
class EvaluatorOCCRF:public Evaluator
{
public:
  EvaluatorOCCRF();
  EvaluatorOCCRF(InferenceEngine* infEngine, FeatureGenerator* featureGen);
  ~EvaluatorOCCRF();
  double computeError(DataSequence* X, Model* m);
  virtual double computeError(DataSet* X, Model* m){
    return Evaluator::computeError(X,m);
...
class EvaluatorOCHCRF:public Evaluator
{
public:
  EvaluatorOCHCRF();
  EvaluatorOCHCRF(InferenceEngine* infEngine, FeatureGenerator* featureGen);
  ~EvaluatorOCHCRF();
  double computeError(DataSequence* X, Model* m);
  virtual double computeError(DataSet* X, Model * m){
    return Evaluator::computeError(X,m);
