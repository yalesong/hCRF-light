...
// Single-view, Hierarchical Sequence Summarization Models
...
{  
  cerr << "HCRF-light library ver 2.1"<< endl;
  cerr << "  Includes CRF, HCRF, LDCRF, MV-HCRF, MV-LDCRF, HSS-HCRF"<< endl;
  cerr << endl;
  ...
  cerr << " -m\t Select the model type (hcrf, ldcrf, mvhcrf, mvldcrf, hsshcrf) (def. = hsshcrf)" << endl;
  ...
  cerr << " -s\t Sigma2: L2-norm regularization factor (def. = 0.0)" << endl;
  ...
  cerr << "   > testModel -m hcrf -h 4 -Fd data.csv -Fq label.csv -FD data.csv -DQ label.csv" << endl;
  cerr << " HCNF with nbHiddenStates = 4, nbGates = 8" << endl;
  cerr << "   > testModel -m hcrf -h 4 -g 8 -Fd data.csv -Fq label.csv -FD data.csv -DQ label.csv" << endl;
  cerr << " LDCRF with nbHiddenStates = 4" << endl;
  cerr << "   > testModel -m ldcrf -h 4 -Fd data.csv -Fl label.csv -FD data.csv -DL label.csv" << endl;
  cerr << " HSS-HCRF with nbHiddenStates = 4, nbGates = 0, maxFeatureLayer = 4" << endl;
  cerr << "   > testModel -m hsshcrf -h 4 -g 0 -HL 4 -HT 0.1 -Fd data.csv -Fq label.csv -FD data.csv -DQ label.csv" << endl;
  cerr << " HSS-HCNF with nbHiddenStates = 4, nbGates = 8, maxFeatureLayer = 4" << endl;
  cerr << "   > testModel -m hsshcrf -h 4 -g 8 -HL 4 -HT 0.1 -Fd data.csv -Fq label.csv -FD data.csv -DQ label.csv" << endl;
  exit(1);
}
...
{   
	...
	string home = ".\\";
#endif
	string fDataTrain      = home + "dataTrain.csv";
	string fLabelsTrain    = home + "labelsTrain.csv" ;
	string fSeqLabelsTrain = home + "seqLabelsTrain.csv";
	string fDataTest       = home + "dataTest.csv";
	string fLabelsTest     = home + "labelsTest.csv";
	string fSeqLabelsTest  = home + "seqLabelsTest.csv";
	string fModel          = home + "model.txt";
	string fFeatures       = home + "features.txt";
	string fOutput         = home + "results.txt";
	string fStats          = home + "stats.txt";
