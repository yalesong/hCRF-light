...
//
//	This class is included in the matrix.h header file.
//
//	February 2, 2006
...
int Matrix<elType>::write(std::ostream* stream) const
{
	...
	for(row=0;row<height;row++) {
		for(col=0;col<width;col++) {
			(*stream) << setiosflags(std::ios::fixed) << std::setprecision(dispPrecision) << getValue(row,col);  //TODO: Doesn't work with unsigned char
			if(stream->fail()) return 1;
...
void Matrix<elType>::display(std::ostream* stream) const
{
	...
	for(row=0;row<height;row++) {
		...
		for(col=0;col<width;col++) {
			(*stream) << setiosflags(std::ios::fixed) << std::setprecision(dispPrecision) << getValue(row,col) << " ";
		}
		if (row< height-1) {
			(*stream) << "\n";
		}else {
